...
def load(filename, start=None, end=None, order=None, world=False):
    ...
    for line in f:
        ...
        if fmatch:
            ...
            euler_rotations = np.zeros((fnum, len(orients), 3))
            continue
        ...
        if dmatch:
            ...
            if   channels == 3:
                ...
                euler_rotations[fi, : ] = data_block[3: ].reshape(N,3)
            elif channels == 6:
                data_block = data_block.reshape(N,6)
                positions[fi,:] = data_block[:,0:3]
                euler_rotations[fi,:] = data_block[:,3:6]
            elif channels == 9:
                positions[fi,0] = data_block[0:3]
                data_block = data_block[3:].reshape(N-1,9)
                euler_rotations[fi,1:] = data_block[:,3:6]
    ...
    quat_rotations = Quaternions.from_euler(np.radians(euler_rotations), order=order, world=world)
    
    return (Animation(quat_rotations, positions, orients, offsets, parents, order, euler_rotations), names, frametime)
