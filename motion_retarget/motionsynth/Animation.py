...
class Animation:
    ...
    def __init__(self, rotations, positions, orients, offsets, parents, order=None, euler_rotations=None):
        ...
        self.order     = order
        self.euler_rotations = euler_rotations
    
    def update_rotations(self, euler_rotations):
        assert self.order is not None
        assert euler_rotations.ndim == 3
        assert len(euler_rotations) == len(self.euler_rotations)
        self.euler_rotations = euler_rotations
        self.rotations = Quaternions.from_euler(np.radians(euler_rotations), order=self.order, world=False)
    ...
    def copy(self): return Animation(
        ...
        self.parents.copy(), self.order, self.euler_rotations.copy())
